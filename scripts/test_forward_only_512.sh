...
#SBATCH --job-name=test_forward_only_512
#SBATCH --output=tests/outputs/log/test_forward_only_512_%j.out
#SBATCH --error=tests/outputs/log/test_forward_only_512_%j.err
#SBATCH --nodes=1
...
srun singularity run \
  ...
  "${SIF}" \
  bash -c "PYTHONPATH=${PROJECT_DIR} python3 -u ${PROJECT_DIR}/tests/run_forward_only.py lumi 512"
